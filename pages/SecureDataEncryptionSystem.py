...
if "login_failed" not in st.session_state:
    st.session_state.login_failed = 0
...
# === Login === #
    ...
    if time.time() < st.session_state.lockout_time:
        ...
        st.error(f"Too many failed attempts! Please wait {remaining} seconds ⏱️")
        st.stop()
    ...
    if st.button("Login"):
        if username in stored_data and stored_data[username]["password"] == hash_password(password):
            ...
            st.session_state.login_failed += 1
            remaining = 3 - st.session_state.login_failed
            st.error(f"❌ Invalid Credentials! Attempts left: {remaining}")

            if st.session_state.login_failed >= 3:
                st.session_state.lockout_time = time.time() + LOGOUT_DURATION
                st.error("🛑 Too many failed attempts. Locked for 69 seconds.")
                st.stop()
